...
const CartPage = () => {
  const { cartItems, editCartQuantity, removeCartItem, clearCart } = useCartStore();
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 p-4">
        ...
        <Link href="/user/products">
          <Button size="lg" className="mt-4 rounded-xl px-8 shadow-sm">Start Shopping</Button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto w-full animate-in fade-in zoom-in-95 duration-300">
      <div className="flex items-center justify-between mb-8">
        ...
        <Button variant="ghost" className="text-destructive hover:bg-destructive/10 rounded-xl" onClick={clearCart}>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 flex flex-col gap-4">
          {cartItems.map((item) => (
            <Card key={item.id} className="flex flex-col sm:flex-row items-center sm:items-start p-4 gap-4 sm:gap-6 relative rounded-2xl shadow-sm border-border hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 flex flex-col h-full py-2 w-full">
                <div className="flex justify-between items-start gap-4">
                  <div>
                    ...
                    <p className="text-muted-foreground text-sm line-clamp-2 mt-1 pr-4">{item.description}</p>
                  </div>
                  <span className="font-bold text-xl text-primary whitespace-nowrap">${item.price}</span>
                </div>
                ...
                <div className="mt-auto pt-6 flex items-center justify-between w-full">
                  <div className="flex items-center gap-3">
                    <Button 
                      ...
                      onClick={() => item.quantity > 1 ? editCartQuantity(item.id, item.quantity - 1) : removeCartItem(item.id)}
                    >
                      <Minus className="h-4 w-4" />
                    </Button>
                    <span className="w-6 text-center font-semibold text-lg">{item.quantity}</span>
                    <Button 
                      size="icon" 
                      variant="outline" 
                      className="h-8 w-8 rounded-full border-muted-foreground/30 hover:border-primary hover:text-primary transition-colors" 
                      onClick={() => editCartQuantity(item.id, item.quantity + 1)}
                    >
                      <Plus className="h-4 w-4" />
                    </Button>
                  </div>
                  <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg" onClick={() => removeCartItem(item.id)}>
                    Remove
                  </Button>
                </div>
              </div>
          ...
          <Card className="sticky top-28 bg-card shadow-lg border-border/50 rounded-2xl overflow-hidden">
            ...
            <CardFooter className="pb-6 flex flex-col gap-4">
              <div className="w-full space-y-2 pb-2">
                ...
                {addresses.length === 0 && (
                  <Link href="/user/addresses" className="text-xs text-primary hover:underline block mt-1">
                    + Add a new address
                  </Link>
                )}
              </div>
              ...
              <Link href="/user/products" className="text-sm font-medium text-muted-foreground hover:text-primary transition-colors underline-offset-4 hover:underline">
                Continue Shopping
              </Link>
            </CardFooter>
          </Card>
